...
function albumCount(){
	...
	$numberOfPhotos = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$bp->album->table_name} WHERE owner_id =".$id) );
	$string = "Album <span>".$numberOfPhotos."</span>";
	return $string;
}
function bp_album_setup_nav() {
	...
	$nav_item_name = apply_filters( 'bp_album_nav_item_name', __( albumCount(), 'bp-album' ) );
