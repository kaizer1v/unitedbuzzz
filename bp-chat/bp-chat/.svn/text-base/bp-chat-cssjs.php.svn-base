...
//if user is online, load the javascript
    if(is_user_logged_in()&&!is_admin()){//has issues while loading on admin pages a 0 is appeneded still not sure why ?
        ...
        wp_enqueue_script("cookie",BP_CHAT_PLUGIN_URL."bp-chat/js/tip/jquery.cookie.js",array("jquery"));
        wp_enqueue_script("scroll",BP_CHAT_PLUGIN_URL."bp-chat/js/tip/jquery.scrollTo-min.js",array("jquery"));
