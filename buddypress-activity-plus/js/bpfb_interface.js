...
var BpfbVideoHandler = function () {
	...
	var createMarkup = function () {
		var html = '<input type="text" id="bpfb_video_url" class="inputbox-common-class" name="bpfb_video_url" placeholder="' + l10nBpfb.paste_video_url + '" />' +
			'<input type="button" class="new-topic-ajax-button" id="bpfb_video_url_preview" value="' + l10nBpfb.preview + '" />';
		$container.empty().append(html);
		
		$('#bpfb_video_url').width('480px');
	...
	var createVideoPreview = function () {
		...
		$.post(ajaxurl, {"action":"bpfb_preview_video", "data":url}, function (data) {
			...
			$('.bpfb_action_container').html(
				...
				'<input type="button" class="button new-topic-ajax-button" id="bpfb_cancel" value="' + l10nBpfb.cancel + '" /></p>'
			);
...
var BpfbLinkHandler = function () {
	...
	var createMarkup = function () {
		var html = '<input type="text" id="bpfb_link_preview_url" class="inputbox-common-class" name="bpfb_link_preview_url" placeholder="' + l10nBpfb.paste_link_url + '" />' +
			'<input type="button" class="new-topic-ajax-button" id="bpfb_link_url_preview" value="' + l10nBpfb.preview + '" />';
		$container.empty().append(html);
		
		$('#bpfb_link_preview_url').width('480px');
	...
	var createPreviewMarkup = function (data) {
		...
		$('.bpfb_action_container').html(
			'<p><input type="button" class="button-primary bpfb_primary_button new-topic-ajax-button" id="bpfb_submit" value="' + l10nBpfb.add_link + '" /> ' +
			'<input type="button" class="button new-topic-ajax-button" id="bpfb_cancel" value="' + l10nBpfb.cancel + '" /></p>'
		);
		$("#bpfb_cancel_action").hide();
...
var BpfbPhotoHandler = function () {
	...
	var createMarkup = function () {
		var html = '<div id="bpfb_tmp_photo"> </div>' +
			...
			'<input type="button" class="new-topic-ajax-button" id="bpfb_add_remote_image" value="' + l10nBpfb.add_remote_image + '" /><div id="bpfb_remote_image_container"></div>' +
			'<input type="button" class="new-topic-ajax-button" id="bpfb_remote_image_preview" value="' + l10nBpfb.preview + '" />';
		...
		$("#bpfb_add_remote_image").click(function () {
			...
			$("#bpfb_remote_image_container .bpfb_remote_image").width('480px');
		});
...
function createMarkup () {
	var html = '<div class="bpfb_actions_container">' +
		'<div class="bpfb_toolbar_container">' +
			'<a href="#photos" title="' + l10nBpfb.add_photos + '" id="bpfb_addPhotos"><img src="' + _bpfbRootUrl + '/img/system/camera.png" border="0" />Photos</a>' +
			'&nbsp;' +
			'<a href="#videos" title="' + l10nBpfb.add_videos + '" id="bpfb_addVideos"><img src="' + _bpfbRootUrl + '/img/system/film.png" border="0" />Videos</a>' +
			'&nbsp;' +
			'<a href="#links" title="' + l10nBpfb.add_links + '" id="bpfb_addLinks"><img src="' + _bpfbRootUrl + '/img/system/link.png" border="0" />Links</a>' +
		'</div>' +
		...
		'</div>' +
		'<input type="button" id="bpfb_cancel_action" class="new-topic-ajax-button" value="' + l10nBpfb.cancel + '" style="display:none" />' +
	'</div>';
	$form.wrap('<div class="bpfb_form_container" />');
	$textContainer.before(html);
}
