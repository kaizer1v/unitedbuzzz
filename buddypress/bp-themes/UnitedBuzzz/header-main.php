<?php if ( !is_user_logged_in() && !isset($_GET['twc_oauth_start'])) {
	...
	$myFirstLogin = get_user_meta( $loggedinUserID, 'first_login_check');
	if($myFirstLogin[0] == "false"){
		global $bp;
		update_user_meta( $loggedinUserID, 'first_login_check', 'true' );
		header('Location:'. get_site_url(). '/members/' . $bp->loggedin_user->userdata->user_nicename . '/profile/edit');	
	}
?>
...
<!--[if gt IE 8]><!--> <html class="no-js" lang="en"> <!--<![endif]-->
	<head>
        ...
        <script src="<?php bloginfo('template_url')?>/js/libs/jquery-1.6.2.min.js" type="text/javascript" charset="utf-8"></script>
		...
		<?php if(bp_is_user_profile()){ ?>
			<script type="text/javascript" src="<?php bloginfo('template_url')?>/js/myfancybox.js"></script>
		<?php } ?>
