...
<?php if($check_string == 'its_me'){ ?>
	...
	<form action="<?php bp_activity_post_form_action() ?>" method="post" id="whats-new-form" class="clearfix" name="whats-new-form">
		...
		<h5>
			<?php if ( bp_is_group() ) : ?>
				<?php printf( __( "SHARE IN %s", 'buddypress' ), bp_get_group_name(), bp_get_user_firstname() ) ?>
			<?php else : ?>
				<?php printf( __( "SHARE", 'buddypress' ), bp_get_user_firstname() ) ?>
			<?php endif; ?>
		</h5>
		...
		<div id="whats-new-content">
			...
			<div id="whats-new-options">
				<div id="whats-new-submit">
					...
					<input type="submit" name="aw-whats-new-submit"  class="new-topic-ajax-button" id="aw-whats-new-submit" value="<?php _e( 'Post Update', 'buddypress' ) ?>" />
				</div>
...
<form action="<?php bp_activity_post_form_action() ?>" method="post" id="whats-new-form" class="clearfix" name="whats-new-form">
	...
	<div id="whats-new-content">
		<div id="whats-new-textarea">
			<textarea name="whats-new" id="whats-new" cols="50" rows="10"><?php if ( isset( $_GET['r'] ) ) : ?>@<?php echo esc_attr( $_GET['r'] ) ?> <?php endif; ?><?php echo "@".$bp->displayed_user->userdata->user_nicename." : " ; ?></textarea>
		</div>
